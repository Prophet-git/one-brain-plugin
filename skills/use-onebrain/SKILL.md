...
**Una cosa cerrada, una memoria.** Lo que todavía se está discutiendo no se guarda: si la idea sigue cambiando, esperá a que cierre. Guardar cada versión intermedia deja tres o cuatro memorias que dicen cosas distintas sobre lo mismo, y el que busque dentro de dos semanas no va a saber cuál vale — el ruido lo paga todo el equipo, no la sesión que lo generó. Si algo que guardó la misma persona cambió (un dato, una fecha, un número), no sumes otra memoria: guardá la nueva con `supersedes` apuntando a la vieja, sin preguntar, y gana la más nueva. Si la vieja es de otra persona, `supersedes` se usa cuando la persona confirma el reemplazo.
